// script.js (Versjon 16 / V2.7 - Scenario Btns, Pos Fix, UI Moves, Title/Highlight Fix, History)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let rangeHistory = []; // NY: For range historikk
    let currentHistoryIndex = -1; // NY: Index i rangeHistory
    ...
    const rangeTitleElement = document.getElementById('rangeTitle');
    const rangeGrid = document.getElementById('rangeGrid');
    const potDisplaySpan = document.querySelector('.pot-display span'); // Oppdatert for querySelector
    ...
    const rangeSituationInfo = document.getElementById('rangeSituationInfo'); // Trengs denne fortsatt? Tittel settes i H3
    const statsTableBody = document.querySelector('#statsTable tbody');
    const resetStatsBtn = document.getElementById('resetStatsBtn');
    const showRangeToggle = document.getElementById('showRangeToggle'); // NY: Toggle checkbox
    const showRangeToggleLabel = document.getElementById('showRangeToggleLabel'); // NY: Label for toggle
    const prevRangeBtn = document.getElementById('prevRangeBtn'); // NY: Nav button
    const nextRangeBtn = document.getElementById('nextRangeBtn'); // NY: Nav button
    ...
    const MAX_HISTORY = 10; // Max antall ranges å lagre i historikken
    ...
    // REVIDERT getSeatPosition (V13/V2.5 style - Enklere rotasjon)
    function getSeatPosition(logicalSeatIndex, totalPlayers) {
        ...
        const standardAngle = -90 + logicalSeatIndex * angleIncrement; // Default angle (0 at top)
        const currentVisualHeroAngle = -90 + VISUAL_HERO_SEAT_INDEX * angleIncrement;
        const rotationNeeded = 90 - currentVisualHeroAngle; // Rotate to put visual hero at 90 deg
    ...
    // KORRIGERT TITTEL LOGIKK + HIGHLIGHT + HISTORY
    function handleUserAction(userActionCode) {
        ...
        // Bruk info fra DA hånden startet (lastHandPlayedInfo)
        const infoForThisHand = { ...lastHandPlayedInfo };

        const gtoActionObject = getGtoAction(infoForThisHand.stack, infoForThisHand.players, infoForThisHand.pos, infoForThisHand.scenario, handKey); if (!gtoActionObject) { console.error("Failed GTO obj."); feedbackText.textContent = "Error: No GTO data."; feedbackText.className = 'incorrect'; return; }
        ...
        // --- Oppdater statistikk ---
        updateStatistics(infoForThisHand.pos, isCorrect); saveStatistics(); displayStatistics();

        // --- Vis Range & Start Timer ---
        // Vis alltid range etter action i standard mode, eller hvis toggle er på i trainer mode (selv om den allerede vises)
        if (currentTrainingMode === 'standard' || showRangeToggle.checked) {
            // Vis range for hånden som nettopp ble spilt
            displayRangeGridForSituation(infoForThisHand.stack, infoForThisHand.players, infoForThisHand.pos, infoForThisHand.scenario, infoForThisHand.hand, true); // true = legg til i historikk
            // Tittel settes av displayRangeGridForSituation nå
        }
    ...
    // REVIDERT: Mottar heroHandKey, legger til i historikk
    function displayRangeGridForSituation(stack, players, pos, scenario, heroHandKey, addToHistory = false) {
        const contextKey = `${stack}-${players}-${pos}-${scenario}-${heroHandKey || 'nohand'}`; // Nøkkel for historikk-sjekk

        // Legg til i historikk KUN hvis det er en ny, unik situasjon som vises
        if (addToHistory) {
            const lastHistoryKey = rangeHistory.length > 0 ? rangeHistory[rangeHistory.length - 1].key : null;
            if (contextKey !== lastHistoryKey) {
                const historyEntry = { stack, players, pos, scenario, heroHandKey, key: contextKey };
                rangeHistory.push(historyEntry);
                if (rangeHistory.length > MAX_HISTORY) {
                    rangeHistory.shift(); // Fjern eldste
                }
                currentHistoryIndex = rangeHistory.length - 1; // Pek på den nye siste
                console.log(`Added to history. Index: ${currentHistoryIndex}, Total: ${rangeHistory.length}`);
            } else {
                 // Oppdaterer kanskje bare highlight for samme kontekst
                 currentHistoryIndex = rangeHistory.length - 1; // Sørg for at vi peker på siste
                 console.log("Context already last in history.");
            }
        }
        ...
        // Sett TITTEL basert på om vi viser historikk eller aktiv/forrige
        if (addToHistory) { // Viser en ny "forrige" eller aktiv range
             rangeTitleElement.textContent = (currentTrainingMode === 'trainer' || showRangeToggle.checked) ? `Anbefalt Range (${pos}):` : `Range for Forrige Hånd (${pos}):`;
        } else { // Viser fra historikk
             rangeTitleElement.textContent = `Historikk (${currentHistoryIndex + 1}/${rangeHistory.length}) - Range for ${pos}:`;
        }
        ...
        // Highlight hero's hand
        if(heroHandKey) { const heroCell = rangeGrid.querySelector(`[data-handkey="${heroHandKey}"]`); if(heroCell) { heroCell.classList.add('highlight-hand'); } else { console.warn(`Could not find cell for highlight: ${heroHandKey}`); } }
        else { console.log("No heroHandKey provided to highlight."); }

        updateRangeNavButtons(); // Oppdater forrige/neste knappene
        rangeDisplayContainer.style.display = 'block';
    }

    // --- Range History Navigation ---
    function navigateRangeHistory(direction) {
        clearTimeout(autoNewHandTimer); // Stopp timer når vi blar

        if (rangeHistory.length === 0) return;

        let newIndex = currentHistoryIndex + direction;

        // Begrens til gyldige indekser
        newIndex = Math.max(0, Math.min(rangeHistory.length - 1, newIndex));

        if (newIndex !== currentHistoryIndex) {
            currentHistoryIndex = newIndex;
            const historyEntry = rangeHistory[currentHistoryIndex];
            console.log(`Navigating history to index ${currentHistoryIndex}:`, historyEntry);
            // Vis den historiske rangen UTEN å legge den til i historikken på nytt
            displayRangeGridForSituation(historyEntry.stack, historyEntry.players, historyEntry.pos, historyEntry.scenario, historyEntry.heroHandKey, false);
        }
         updateRangeNavButtons();
    }

    function updateRangeNavButtons() {
        prevRangeBtn.disabled = (currentHistoryIndex <= 0);
        nextRangeBtn.disabled = (currentHistoryIndex >= rangeHistory.length - 1);
    }

    // --- Toggle Visibility Logic ---
    function updateRangeToggleVisibility() {
         if (currentTrainingMode === 'standard') {
             showRangeToggle.style.display = 'inline-block';
             showRangeToggleLabel.style.display = 'inline-block';
         } else { // Skjul for trainer mode
             showRangeToggle.style.display = 'none';
             showRangeToggleLabel.style.display = 'none';
             showRangeToggle.checked = false; // Reset når skjult
         }
    }
    ...
    trainingModeSelect.addEventListener('change', (e) => { clearTimeout(autoNewHandTimer); currentTrainingMode = e.target.value; updateRangeToggleVisibility(); if (currentTrainingMode === 'trainer') { positionLabel.style.display = 'inline'; positionSelect.style.display = 'inline'; populatePositionSelect(); currentFixedPosition = positionSelect.value; } else { positionLabel.style.display = 'none'; positionSelect.style.display = 'none'; currentFixedPosition = null; } setupNewHand(); });
    ...
    prevRangeBtn.addEventListener('click', () => navigateRangeHistory(-1)); // NY
    nextRangeBtn.addEventListener('click', () => navigateRangeHistory(1)); // NY
    showRangeToggle.addEventListener('change', () => { // NY: Håndter toggle endring
         if (currentTrainingMode === 'standard') {
             if (showRangeToggle.checked) {
                 // Vis range for *nåværende* hånd hvis vi er i standard og toggle slås på
                 if (lastHandPlayedInfo.pos) { // Sørg for at vi har en hånd å vise
                      rangeTitleElement.textContent = `Anbefalt Range (${lastHandPlayedInfo.pos}):`;
                      displayRangeGridForSituation(lastHandPlayedInfo.stack, lastHandPlayedInfo.players, lastHandPlayedInfo.pos, lastHandPlayedInfo.scenario, lastHandPlayedInfo.hand, false); // Ikke legg til i historikk
                 }
             } else {
                 // Skjul range hvis toggle slås av (med mindre den allerede viser forrige)
                 if (!feedbackText.textContent) { // Skjul kun hvis vi ikke akkurat har gitt feedback
    ...
    function initializeApp() {
        console.log("Initializing Poker Trainer V2.7...");
        ...
        setupTableUI(); populatePositionSelect(); setupNewHand(); displayStatistics();
        updateRangeNavButtons(); // Init range nav buttons state
        updateRangeToggleVisibility(); // Init toggle visibility
