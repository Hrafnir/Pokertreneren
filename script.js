// script.js (Versjon 14 / V2.6 - Scenario Buttons, Title Fix, Highlight Hand)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let numPlayers = 9; // Default til 9max
    ...
    let statistics = {};
    let currentGameFormat = '9max_tourney'; // NY: Holder styr på valgt format

    // --- DOM Elementer ---
    const scenarioButtonsContainer = document.querySelector('.scenario-buttons'); // NY
    ...
    function handleUserAction(userActionCode) {
        ...
        // Bruk info fra DA hånden startet (lagret i lastHandPlayedInfo)
        ...
        // Pass handKey for highlighting
        displayRangeGridForSituation(infoForRangeDisplay.stack, infoForRangeDisplay.players, infoForRangeDisplay.pos, infoForRangeDisplay.scenario, infoForRangeDisplay.hand);
        rangeTitleElement.textContent = `Range for Forrige Hånd (${infoForRangeDisplay.pos || '--'}):`; // Bruk lagret pos
    ...
    // MODIFIED: Mottar heroHandKey for highlighting
    ...
    // --- Event Listeners ---
    // NY: Listener for scenario buttons (delegation)
    scenarioButtonsContainer.addEventListener('click', (e) => {
        if (e.target.classList.contains('scenario-btn')) {
            ...
            currentGameFormat = scenarioType; // Oppdater global state

            // Sett numPlayers basert på valgt scenario
            if (scenarioType.startsWith('6max')) {
                numPlayers = 6;
            } else if (scenarioType.startsWith('9max')) {
                numPlayers = 9;
            }
            console.log(`Scenario selected: ${scenarioType}, Num Players: ${numPlayers}`);

            // Visuell oppdatering av knapper
            scenarioButtonsContainer.querySelectorAll('.scenario-btn').forEach(btn => btn.classList.remove('active'));
            e.target.classList.add('active');

            // Oppdater UI og start ny hånd
    ...
    function initializeApp() {
        console.log("Initializing Poker Trainer V2.6...");
        calculateVisualHeroSeatIndex();
        initializeStatistics();
        loadStatistics();
        // Set default active scenario button visually
        ...
        if(defaultScenarioBtn) defaultScenarioBtn.classList.add('active');
             ...
             const firstBtn = scenarioButtonsContainer.querySelector('.scenario-btn');
             if(firstBtn) {
                firstBtn.classList.add('active');
                currentGameFormat = firstBtn.dataset.scenarioType;
                numPlayers = currentGameFormat.startsWith('6max') ? 6 : 9;
                calculateVisualHeroSeatIndex();
             }
        }
        setupTableUI();
    ...
    initializeApp(); // Kjør initialiseringen
});
